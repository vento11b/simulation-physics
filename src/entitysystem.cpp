...
void EntitySystem::load() {
    for (Object *object : entities) {
        if (object->mesh->loaded && object->mesh == NULL && object->shader == NULL) continue;
...
void EntitySystem::load(Object* object) {
    if (object->mesh->loaded && object->mesh == NULL && object->shader == NULL) return;
    glGenVertexArrays(1, &object->mesh->VAO);
    glGenBuffers(1, &object->mesh->VBO);
    glGenBuffers(1, &object->mesh->EBO);
...
void EntitySystem::draw(Window window) {
    for (Object* object : entities) {
        if (!object->mesh->loaded) load(object);
        object->draw(window);
    }
}
